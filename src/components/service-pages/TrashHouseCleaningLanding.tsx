      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-6 py-12 md:grid-cols-[190px_1fr]">
        ...
        <div className="space-y-14 text-[17px] leading-8 text-gray-800">
          ...
          <section id="estimate" className="scroll-mt-36">
            ...
            <p>쓰레기집청소 비용은 평수만으로 정하기 어렵습니다.</p>
            <p className="mt-4">같은 원룸이라도 쌓인 물건의 양, 분류에 필요한 시간, 바닥과 주방의 오염 상태에 따라 작업량이 달라집니다.</p>
            ...
            <p className="mt-2">쓰레기가 봉투에 담겨 있는 공간과 생활용품·서류·음식물이 뒤섞인 공간은 정리에 필요한 시간이 다릅니다.</p>
            <p className="mt-2">넓은 집이라도 반출할 물건이 적으면, 물건이 빽빽하게 쌓인 작은 집보다 작업량이 적을 수 있습니다.</p>
            ...
            <p className="mt-2">찐청소의 쓰레기집청소는 수거·폐기물 처리·청소·소독·냄새 제거가 기본에 포함됩니다.</p>
            <p className="mt-2">다만 기본 포함이라는 말이 폐기물의 양이나 작업 범위에 관계없이 같은 금액이라는 뜻은 아닙니다. 상담 때 확인한 현장 상태와 처리 범위를 기준으로 견적을 산정합니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">다른 견적과 비교하실 때도 금액뿐 아니라 폐기물 처리, 소독, 냄새 제거가 어디까지 포함되어 있는지 함께 확인해 주세요.</p>

            <h3 className="mt-6 font-bold text-brand-dark">사진만으로도 상담할 수 있나요?</h3>
            <p className="mt-2">사진을 보내주시면 작업 범위와 예상 비용을 상담하는 데 도움이 됩니다.</p>
            <p className="mt-2">방 전체 모습, 주방과 욕실, 쓰레기가 집중된 곳, 큰 가구나 가전이 함께 보이면 좋습니다.</p>
            <p className="mt-2">다만 사진에 가려진 바닥이나 수납장 내부 등은 확인이 어려워, 현장 확인이 필요한 경우도 있습니다.</p>
          </section>
          ...
          <section id="scope" className="scroll-mt-36">
            ...
            <p className="mt-4 text-[15px] text-gray-500">위 항목은 기본 작업에 자동으로 포함되는 것으로 보지 않으며, 진행 가능 여부와 비용을 사전에 확인합니다.</p>
          </section>
          ...
          <section id="extra" className="scroll-mt-36">
            ...
            <p>수거·폐기물 처리·소독·냄새 제거는 기본 포함 항목입니다.</p>
            <p className="mt-4">추가 비용 여부는 해당 작업의 명칭보다, 처음 협의한 물량과 범위가 달라지는지를 기준으로 확인해야 합니다.</p>
            <p className="mt-4 font-bold text-brand-dark">다음과 같은 경우에는 견적이 달라질 수 있습니다.</p>
            <ul className="mt-3 space-y-2.5">
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">견적 상담 시 기본 포함 항목, 별도 요청 항목, 현장 확인이 필요한 항목을 함께 확인해 주세요.</p>
          </section>

          {/* 4. 진행순서 */}
          <section id="process" className="scroll-mt-36">
            <SectionTitle id="process-title" kicker="04" title="청소 진행 순서와 소요 시간" />
            <p>기본 진행 흐름은 상담 → 범위·견적 확인 → 분류·반출 → 청소 → 소독·냄새 제거 → 검수입니다.</p>
            ...
            <p className="mt-2">소요 시간은 쓰레기의 양, 분류 작업, 오염 정도, 반출 조건에 따라 달라집니다.</p>
            <p className="mt-2">평수만으로 당일 완료를 약속하기보다는, 현장을 확인한 뒤 예상 일정을 안내합니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">퇴실이나 이사처럼 정해진 기한이 있다면 상담할 때 먼저 말씀해 주세요.</p>
          </section>
          ...
          {cases.length > 0 && (
            <section id="cases" className="scroll-mt-36">
              ...
              <p>청소가 끝났다는 설명만으로는 집 상태를 충분히 알기 어렵습니다. 찐청소는 작업 전후 사진을 자세히 촬영해 보내드립니다.</p>
              <div className="mt-6">
                <CaseGallery items={cases} />
              </div>
              <p className="mt-5 font-bold text-brand-dark">전체 모습과 함께 아래 항목을 확인해 주세요.</p>
              <ul className="mt-3 space-y-2.5">
              ...
              <p className="mt-4">비대면으로 맡기시는 경우, 특히 확인하고 싶은 구역을 미리 알려주세요.</p>
              <p className="mt-4 text-[15px] text-gray-500">사진은 눈에 보이는 상태를 확인하는 자료입니다. 냄새나 자재 내부 상태까지 사진만으로 판단할 수는 없으므로, 이런 부분은 작업 설명과 함께 확인하는 것이 좋습니다.</p>
            </section>
          )}
          ...
          <section id="area" className="scroll-mt-36">
            ...
            <p>현장 주소와 희망 날짜를 알려주시면 방문 가능 여부와 일정을 안내합니다.</p>
            <p className="mt-4">층수, 엘리베이터 유무, 주차·반출 여건도 함께 알려주시면 상담에 도움이 됩니다.</p>

            <h3 className="mt-6 font-bold text-brand-dark">현장에 없어도 맡길 수 있나요?</h3>
            <p className="mt-2">네. 원하시면 비대면으로 진행할 수 있습니다.</p>
            <p className="mt-2">작업 전후 사진도 상세하게 전달해 드립니다.</p>
            <p className="mt-4 font-bold text-brand-dark">비대면 진행 전에는 다음 내용을 협의합니다.</p>
            <ul className="mt-3 space-y-2.5">
            ...
            <div className="mt-6 rounded-xl bg-gray-50 p-5">
              ...
              <p className="mt-2 text-[15.5px]">방문 시간이나 반출 동선 등 걱정되는 부분을 상담할 때 알려주세요. 현장 조건에서 조정할 수 있는 방법을 함께 확인하겠습니다.</p>
              <p className="mt-2 text-[15px] text-gray-500">다만 공동현관, 복도, 엘리베이터 등을 이용해야 하므로 주변에서 작업 사실을 전혀 알 수 없다고 약속드리기는 어렵습니다.</p>
            </div>
          </section>
          ...
          <section id="checkup" className="scroll-mt-36">
            ...
            <p>검수는 처음 협의한 범위를 기준으로 진행합니다.</p>
            <p className="mt-4">집이 비워졌는지뿐 아니라, 요청한 청소와 정리가 어디까지 이루어졌는지 확인해 주세요.</p>
            <p className="mt-4 font-bold text-brand-dark">검수 시 확인할 항목</p>
            <ul className="mt-3 space-y-2.5">
            ...
            <p className="mt-4">작업 후 궁금한 부분이 있다면 해당 구역의 사진과 함께 문의해 주세요.</p>
            <p className="mt-2 text-[15px] text-gray-500">재확인이나 사후 처리의 적용 범위와 기간은 계약 시 확인하시기 바랍니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">청소로 제거할 수 있는 오염과 자재 자체의 변색·파손은 구분해서 안내받는 것이 중요합니다.</p>
          </section>

          {/* 8. 준비사항 */}
          <section id="prep" className="scroll-mt-36">
            <SectionTitle id="prep-title" kicker="08" title="청소 전에 준비할 사항" />
            <p>상담을 받기 위해 쓰레기를 미리 정리하거나 봉투에 모두 담아두실 필요는 없습니다. 가능한 범위에서 아래 내용만 알려주세요.</p>

            <h3 className="mt-6 font-bold text-brand-dark">남길 물건을 알려주세요</h3>
            <p className="mt-2">신분증, 통장, 계약서, 사진, 귀중품 등 보관할 물건의 목록을 전달해 주세요. 직접 꺼내기 어렵다면 무리해서 찾기보다 특징과 예상 위치를 알려주시면 됩니다.</p>

            <h3 className="mt-6 font-bold text-brand-dark">현재 상태를 보여주세요</h3>
            <p className="mt-2">사진을 찍을 수 있다면 방 전체와 주요 오염 구역을 보내주세요. 사진 촬영이 어렵다면 그 상황부터 말씀해 주세요.</p>

            <h3 className="mt-6 font-bold text-brand-dark">출입과 반출 조건을 알려주세요</h3>
            <p className="mt-2">층수, 엘리베이터, 주차 위치, 건물의 작업 가능 시간 등을 확인해 주세요. 관리실과의 사전 협의가 필요한지도 살펴보시면 좋습니다.</p>

            <h3 className="mt-6 font-bold text-brand-dark">작업 후 원하는 상태를 알려주세요</h3>
            <p className="mt-2">계속 거주할 예정인지, 이사 때문에 집을 비워야 하는지에 따라 남길 물건과 마무리 범위가 달라집니다.</p>
            <p className="mt-3 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">&ldquo;쓰레기는 전부 버리되 가구는 남기고 싶어요.&rdquo;<br />&ldquo;퇴실 예정이라 큰 가구도 함께 처리하고 싶어요.&rdquo;<br /><span className="mt-2 block text-gray-700">이렇게 구체적으로 알려주시면 범위를 정하기가 수월합니다.</span></p>
          </section>
          ...
          <section id="contact" className="scroll-mt-36">
            <div className="rounded-2xl bg-brand-dark p-7 text-white">
              ...
              <p className="mt-3 text-white/80">어디부터 설명해야 할지 모르겠다면, 현장 위치와 현재 상태부터 알려주세요. 사진이 있으면 도움이 되지만, 모든 내용을 완벽하게 정리해서 문의하실 필요는 없습니다.</p>
              ...
              <p className="mt-4 text-white/80">찐청소는 쓰레기 수거와 폐기물 처리부터 청소·소독·냄새 제거까지 진행합니다. 필요한 작업과 견적에 포함되는 범위를 확인한 뒤 결정하세요.</p>
