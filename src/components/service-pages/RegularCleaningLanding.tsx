    ...
    <article>
      ...
      <section className="relative overflow-hidden px-6 py-14 text-white md:py-20">
        <div className="absolute inset-0">
          <Image src="/images/hero-bg/regular-hero.webp" alt="" fill priority className="object-cover" sizes="100vw" />
      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-6 py-12 md:grid-cols-[190px_1fr]">
        ...
        <div className="space-y-14 text-[17px] leading-8 text-gray-800">
          ...
          <section id="safety" className="scroll-mt-36">
            ...
            <p className="mt-4">공기질 정화, 냄새제거에 사용되는 약품은 해외 공인 시험기관의 검증을 거친 제품입니다. 사람이 계속 머무는 공간이라 저희도 이 부분을 가장 신경 씁니다.</p>
          ...
          <section id="estimate" className="scroll-mt-36">
            ...
            <p>찐청소의 정기청소 비용은 필요한 인원과 작업 시간을 기준으로 산정합니다.</p>
            <p className="mt-4">여기에 방문 주기와 회차별 작업 범위, 현장 조건을 반영합니다.</p>
            <p className="mt-4">평수가 같아도 공간을 이용하는 사람의 수, 집기 배치, 화장실 유무, 청소할 항목에 따라 필요한 작업량이 달라집니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">평수보다 실제로 할 일을 확인합니다</h3>
            <p className="mt-2">집기가 빽빽한 50평 사무실과 비어 있는 100평 공간은 청소 동선부터 다릅니다.</p>
            <p className="mt-2">화장실과 탕비실을 함께 관리하는지, 바닥만 청소하는지에 따라서도 필요한 인원과 시간이 달라집니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">면적은 참고 자료입니다. 견적의 기준은 정해진 시간에 실제로 해야 할 작업입니다.</p>
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">월 비용을 비교할 때는 방문 횟수, 회당 인원과 시간, 포함 범위를 함께 확인해 주세요.</p>

            <h3 className="mt-8 text-lg font-bold text-brand-dark">같은 월 비용이어도 관리 내용은 다를 수 있습니다</h3>
            <p className="mt-2">짧게 자주 방문하는 방식과 한 번에 여러 구역을 충분히 청소하는 방식은 다릅니다.</p>
            <p className="mt-2">어느 방식이 무조건 좋은 것이 아니라 공간의 사용 패턴과 필요한 작업에 맞아야 합니다.</p>
            <p className="mt-2">찐청소는 요청하신 관리 수준과 현장 상태를 함께 살펴 인원·시간·주기를 정합니다.</p>
          </section>
          ...
          <section id="scope" className="scroll-mt-36">
            ...
            <p className="mt-3 font-bold text-brand-dark">매회 관리할 항목의 예</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {everyVisitExamples.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-5 font-bold text-brand-dark">주기적으로 배정할 항목의 예</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {periodicExamples.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-4 text-[15px] text-gray-500">위 내용은 구성 예시입니다. 실제 포함 항목과 주기는 현장별로 정합니다.</p>

            <div className="mt-6 rounded-xl bg-gray-50 p-5">
              <p className="font-bold text-brand-dark">일반 정기관리와 구분할 작업</p>
              ...
              <p className="mt-4 text-[15px] text-gray-500">필요한 작업은 정기관리와 함께 상담할 수 있지만, 포함 여부와 비용은 별도로 정합니다.</p>
            </div>
          </section>
          ...
          <section id="extra" className="scroll-mt-36">
            ...
            <p className="mt-2">주 몇 회가 적당한지는 평수만으로 정하기 어렵습니다.</p>
            <p className="mt-2">이용자가 많은 공간, 음식물을 사용하는 공간, 외부 먼지가 자주 들어오는 곳은 관리할 내용이 다릅니다.</p>
            <p className="mt-4 font-bold text-brand-dark">상담 시 다음 사항을 함께 확인합니다.</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
            ...
            <p className="mt-2">오랫동안 쌓인 오염이 있다면 짧은 정기관리 시간만으로 정리하기 어려울 수 있습니다.</p>
            <p className="mt-2">이런 경우에는 초기 대청소가 필요한지 먼저 확인합니다. 모든 현장에 초기 대청소를 일괄 적용하는 것은 아닙니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">필요하다면 초기 작업과 이후 정기관리의 범위·비용을 나누어 안내합니다.</p>
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">추가 요청이 기존 시간 안에 가능한지, 작업 시간을 늘려야 하는지 확인하고 범위와 비용을 정합니다.</p>
          </section>
          ...
          <section id="process" className="scroll-mt-36">
            ...
            <p className="mt-2">면적, 청소 항목, 집기 배치, 오염 정도와 투입 인원에 따라 달라집니다.</p>
            <p className="mt-2">인원과 시간을 함께 보아야 합니다. 작업 시간이 같더라도 한 명이 하는 경우와 여러 명이 구역을 나누는 경우는 다릅니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">계약 전에 회당 인원·시간과 작업 범위를 함께 확인해 주세요.</p>
          </section>

          {/* 5. 결과/관리상태 확인 */}
          <section id="cases" className="scroll-mt-36">
            <SectionTitle id="cases-title" kicker="05" title="작업 결과와 관리 상태 확인" />
            <p>정기청소는 한 번의 극적인 전후 사진보다 약속한 관리가 꾸준히 이루어지는지가 중요합니다.</p>
            <p className="mt-4 font-bold text-brand-dark">다음과 같은 부분을 기준으로 확인하면 좋습니다.</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {caseChecklist.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-5 text-[15px] text-gray-500">청소 직후의 상태와 다음 방문 전까지 생긴 오염은 구분해서 살펴야 합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">작업 사진이나 확인 자료가 필요하면 촬영 가능 구역과 제공 방식 등을 상담 시 확인해 주세요. 개인정보나 업무 자료가 보이는 곳은 촬영 제한도 함께 정합니다.</p>
          ...
          <section id="area" className="scroll-mt-36">
            ...
            <p>현장 주소와 원하는 요일·시간대를 알려주시면 서비스 가능 여부와 일정을 확인합니다.</p>
            <p className="mt-4">운영 중에 작업할지, 출근 전이나 영업 종료 후에 진행할지도 현장에 맞춰 정합니다.</p>
            <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
              {reservationChecklist.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-5">이른 시간, 야간, 주말 작업은 희망 조건을 알려주시면 가능 여부를 안내합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">공휴일 방문, 일정 변경, 임시 휴무 때의 처리 방식도 계약 전에 확인해 두는 것이 좋습니다.</p>
          </section>
          ...
          <section id="checkup" className="scroll-mt-36">
            ...
            <p className="mt-2">청소 결과는 계약에 포함된 구역과 항목을 기준으로 확인합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">&lsquo;전체적으로 깨끗하게&rsquo;라는 표현보다 바닥, 화장실, 창틀 등 확인할 항목이 구체적일수록 서로의 기대를 맞추기 쉽습니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">미흡한 부분은 구체적으로 알려주세요</h3>
            <p className="mt-2">작업 범위 안에서 추가 확인이 필요한 곳이 있다면 위치와 상태, 확인 시점을 알려주세요.</p>
            <p className="mt-2 text-[15px] text-gray-500">작업 내용과 현장 상태를 살펴 후속 처리 방법을 안내합니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">반복되는 문제는 범위와 시간을 함께 봅니다</h3>
            <p className="mt-2">특정 구역이 계속 부족하게 느껴진다면 작업 누락인지, 오염이 빠르게 다시 생기는지, 배정한 시간이 부족한지 구분해야 합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">이용 인원이나 운영 방식이 바뀌었다면 방문 주기와 작업 시간을 조정할 필요가 있는지도 확인합니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">추가 요청은 기존 범위와 구분합니다</h3>
            <p className="mt-2">기존 작업의 미흡한 부분을 확인하는 것과 새로운 청소를 추가하는 것은 다릅니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">추가 요청이 생기면 수행 가능 여부와 필요한 시간·비용을 확인해 안내합니다.</p>
          </section>
          ...
          <section id="prep" className="scroll-mt-36">
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">청소 범위를 잘 정해 두면 매번 같은 설명을 반복할 일이 줄어듭니다. 처음에 조금 구체적으로 맞추고, 이후에는 편하게 관리받을 수 있도록 준비하겠습니다.</p>
          </section>
          ...
          <section id="contact" className="scroll-mt-36">
            <div className="rounded-2xl bg-brand-dark p-7 text-white">
              <p className="text-xl font-bold">정기청소 견적 문의</p>
              <p className="mt-3 text-white/80">몇 평인지와 함께, 지금 어떤 관리가 필요한지 알려주세요.</p>
              <p className="mt-2 text-white/80">바닥만 정기적으로 관리하면 되는지, 화장실과 탕비실까지 필요한지, 직원들이 직접 하고 있는 청소를 어디까지 맡기고 싶은지에 따라 구성이 달라집니다.</p>
