      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-6 py-12 md:grid-cols-[190px_1fr]">
        ...
        <div className="space-y-14 text-[17px] leading-8 text-gray-800">
          ...
          <section id="safety" className="scroll-mt-36">
            ...
            <p className="mt-4">공기질 정화, 냄새제거에 사용되는 약품은 해외 공인 시험기관의 검증을 거친 제품입니다. 좁은 공간에서 바로 생활을 시작하시는 경우가 많아 저희도 이 부분을 가장 신경 씁니다.</p>
          ...
          <section id="estimate" className="scroll-mt-36">
            ...
            <p>원룸청소 비용은 평수만으로 정하기 어렵습니다.</p>
            <p className="mt-4">같은 면적이라도 주방과 욕실의 오염, 복층 유무, 옵션 가전, 남아 있는 짐에 따라 필요한 작업이 달라집니다.</p>
            <p className="mt-4">찐청소는 필요한 인원과 장비·약품을 중심으로 실제 작업량을 확인해 견적을 안내합니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">면적이 작아도 기본적으로 필요한 작업이 있습니다</h3>
            <p className="mt-2">원룸에도 주방과 욕실, 창문과 배수구 등 각각 확인할 곳이 있습니다.</p>
            <p className="mt-2">넓은 집보다 바닥 면적은 작더라도 준비와 장비 반입, 공간별 세척과 마무리에는 시간이 필요합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">그래서 큰 집의 청소 비용을 평수 비율로 나눈 금액과 같지는 않을 수 있습니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">같은 원룸도 구조와 상태가 다릅니다</h3>
            <p className="mt-2">빈 원룸과 침대·책상·생활용품이 놓인 원룸은 접근 조건이 다릅니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">복층이나 별도 베란다, 수납 공간이 많다면 작업 범위도 달라집니다. 오래된 집인지보다 현재 오염과 요청 내용을 함께 살펴봅니다.</p>
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">현재 상태를 보여주는 전체 사진과 오염 부위 사진을 함께 보내주시면 상담이 수월합니다.</p>
          </section>
            ...
            <div className="space-y-6">
              {situationItems.map(([title, paragraphs]) => (
                <div key={title}>
                  ...
                  {paragraphs.map((p, i) => (
                    <p key={p} className={i === 0 ? "mt-2" : i === paragraphs.length - 1 ? "mt-2 text-[15px] text-gray-500" : "mt-2"}>{p}</p>
                  ))}
                </div>
              ))}
            </div>
          ...
          <section id="extra" className="scroll-mt-36">
            ...
            <p>처음 정한 범위보다 작업이 늘어나거나 별도 처리가 필요한 경우 견적이 달라질 수 있습니다.</p>
            ...
            <p className="mt-2">공간 청소와 물품 폐기는 다른 작업입니다.</p>
            <p className="mt-2">생활 쓰레기나 음식물, 가구 등이 많이 남아 있다면 사진과 함께 알려주세요. 종류와 양에 따라 일반 원룸청소와 별도 처리가 필요한 범위를 구분합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">남길 물건과 버릴 물건은 미리 정해 주세요. 남아 있다는 이유만으로 임의로 폐기하지 않습니다.</p>
          </section>
          ...
          <section id="process" className="scroll-mt-36">
            ...
            <p className="mt-2">면적이 작아도 오염과 옵션 가전, 짐의 상태에 따라 시간이 달라집니다.</p>
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">사진과 요청 범위를 확인한 뒤 예상 시간을 안내합니다. 작은 원룸이라는 이유만으로 짧은 시간 안에 끝난다고 약속하지 않습니다.</p>
          </section>

          {/* 5. 전후사진/사례 */}
          <section id="cases" className="scroll-mt-36">
            <SectionTitle id="cases-title" kicker="05" title="작업 전후 사진과 결과 확인" />
            <p>원룸청소 결과는 방 전체 사진과 세부 구역을 함께 확인하는 것이 좋습니다.</p>
            <ul className="mt-4 grid gap-2.5 sm:grid-cols-2">
              {caseChecklist.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-5 text-[15px] text-gray-500">겉으로 정돈된 모습과 실제 오염 제거 결과는 구분해서 살펴보세요.</p>
            <p className="mt-2 text-[15px] text-gray-500">사진 제공이 필요하면 촬영할 구역과 전달 가능 여부를 상담 시 확인해 주세요. 개인 문서와 사진, 생활용품 등 촬영을 원하지 않는 대상도 알려주세요.</p>
          </section>
          ...
          <section id="area" className="scroll-mt-36">
            ...
            <p className="mt-4">원룸청소는 짐이 빠지는 시간과 들어오는 시간을 함께 맞추는 것이 중요합니다.</p>
            ...
            <p className="mt-5 text-[15px] text-gray-500">당일 이사와 청소를 함께 진행하려면 실제로 확보되는 작업 시간을 먼저 확인해야 합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">거주 중이라면 작업 중 머물 공간과 외출 계획, 반려동물의 동선도 함께 알려주세요.</p>
          </section>
          ...
          <section id="checkup" className="scroll-mt-36">
            ...
            <p className="mt-2">주방, 욕실, 창틀, 바닥, 옵션 가전 등 견적에 포함된 항목을 확인합니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">수납장과 가전은 외부만인지 내부까지인지 구분해서 살펴봅니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">오염과 기존 손상은 다릅니다</h3>
            <p className="mt-2">오래된 변색, 마루 눌림, 금속 부식, 실리콘 손상 등은 청소만으로 원래 상태가 되지 않을 수 있습니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">청소가 필요한 부분과 수리·교체가 필요한 상태를 구분해 안내합니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">퇴실 확인과 비용 정산은 별개입니다</h3>
            <p className="mt-2">퇴실을 위한 요청 항목이 있다면 사전에 공유해 주세요.</p>
            <p className="mt-2 text-[15px] text-gray-500">다만 청소를 마쳤다는 사실만으로 임대인의 모든 요구가 충족되거나 보증금 반환이 보장되는 것은 아닙니다. 작업 범위와 결과를 확인하는 서비스로 구분해 안내합니다.</p>
            ...
            <p className="mt-4 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">작업 내용과 현장을 확인해 후속 처리 방법을 안내합니다. 접수와 처리 조건은 예약 전에 확인해 주세요.</p>
          </section>
          ...
          <section id="prep" className="scroll-mt-36">
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">사진을 보내려고 먼저 대청소하실 필요는 없습니다. 현재 상태를 보여주시는 편이 작업 범위를 정하는 데 도움이 됩니다.</p>
          </section>
          ...
          <section id="contact" className="scroll-mt-36">
            <div className="rounded-2xl bg-brand-dark p-7 text-white">
              <p className="text-xl font-bold">원룸청소 견적 문의</p>
              <p className="mt-3 text-white/80">몇 평인지와 함께, 지금 방이 어떤 상태인지 알려주세요.</p>
              <p className="mt-2 text-white/80">처음 입주할 빈방인지, 살고 있는 방인지, 퇴실 후 남은 오염을 정리하려는지에 따라 필요한 작업이 달라집니다.</p>
