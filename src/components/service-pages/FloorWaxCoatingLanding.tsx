      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-6 py-12 md:grid-cols-[190px_1fr]">
        ...
        <div className="space-y-14 text-[17px] leading-8 text-gray-800">
          ...
          <section id="estimate" className="scroll-mt-36">
            ...
            <p>바닥왁스코팅 비용은 평수만으로 정하기 어렵습니다. 같은 면적이라도 기존 왁스의 상태, 오염 정도, 집기 유무와 필요한 준비 작업에 따라 작업량이 달라집니다.</p>
            ...
            <p className="mt-2">같은 &lsquo;왁스코팅&rsquo; 견적이라도 포함된 공정은 다를 수 있습니다.</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {includedCheckItems.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-4 text-[15px] text-gray-500">총액을 비교하기 전에 내 바닥에 필요한 공정이 포함되어 있는지 확인하는 것이 좋습니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">제품과 도포 횟수도 작업 조건에 맞춰 확인합니다</h3>
            <p className="mt-2">도포 횟수가 많다는 이유만으로 무조건 좋은 결과를 보장하지는 않습니다. 바닥과 기존 마감에 맞는 제품인지, 제품 기준에 맞춰 준비와 건조가 이루어지는지가 함께 중요합니다.</p>
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">사용할 제품과 도포 계획은 현장 상태를 확인한 뒤 상담에서 안내받으세요.</p>
          </section>
          ...
          <section id="scope" className="scroll-mt-36">
            ...
            <p className="mt-5 text-[15px] text-gray-500">위 항목은 왁스코팅에 자동으로 포함되는 것으로 보지 않으며, 진행 가능 여부와 비용을 사전에 확인합니다.</p>
          </section>

          {/* 3. 추가비용 */}
          <section id="extra" className="scroll-mt-36">
            <SectionTitle id="extra-title" kicker="03" title="추가 비용이 발생할 수 있는 경우" />
            <p>견적은 처음 확인한 바닥 상태와 작업 범위를 기준으로 정합니다. 현장 조건이나 요청 범위가 달라지면 추가 작업이 필요할 수 있습니다.</p>
            ...
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">박리와 집기 이동 등이 이미 견적에 포함되어 있는지 먼저 확인해 주세요. 작업 중 범위 변경이 필요할 때는 비용과 일정을 어떻게 협의할지도 정해두는 것이 좋습니다.</p>
          </section>
          ...
          <section id="process" className="scroll-mt-36">
            ...
            <p className="mt-4 text-[15px] text-gray-500">구체적인 공정과 반복 도포 여부는 바닥 상태와 사용할 제품에 맞춰 정합니다.</p>
            ...
            <p className="mt-2">겉으로 마른 상태와 일상적인 사용을 견딜 상태를 같은 의미로 보면 안 됩니다. 사용 제품과 도포 조건, 온도·습도·환기 등에 따라 필요한 시간이 달라집니다.</p>
            <p className="mt-4 font-bold text-brand-dark">상담할 때 아래 시점을 구분해 확인해 주세요.</p>
            <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {timingChecklist.map(item => (
                <li key={item} className="rounded-xl border border-gray-100 bg-gray-50/60 px-4 py-3">{item}</li>
              ))}
            </ul>
            <p className="mt-5 rounded-xl bg-amber-50 p-4 text-[15.5px] leading-7">&lsquo;몇 시간 뒤면 모두 가능&rsquo;이라고 한 가지 시간으로 안내하기보다, 실제 사용 계획에 맞춰 확인하는 것이 중요합니다.</p>
          </section>

          {/* 5. 전후사진/사례 */}
          <section id="cases" className="scroll-mt-36">
            <SectionTitle id="cases-title" kicker="05" title="작업 전후 사진과 결과 확인 방법" />
            <p>바닥왁스코팅 사례는 광택이 강한 사진만으로 비교하기 어렵습니다. 조명과 촬영 각도에 따라 같은 바닥도 다르게 보일 수 있기 때문입니다.</p>
            ...
            <p className="mt-5 text-[15px] text-gray-500">바닥이 반짝이는 모습뿐 아니라 준비 과정과 실제 작업 범위를 함께 확인해 주세요. 완료 사진이 필요하다면 촬영 구역과 전달 방법을 상담 시 협의하실 수 있습니다. 사진으로는 끈적임, 건조 상태, 실제 사용감을 모두 판단할 수 없어 현장 확인도 중요합니다.</p>
            {cases.length > 0 && (
          ...
          <section id="area" className="scroll-mt-36">
            ...
            <p className="mt-2">원하는 작업 시간대를 알려주시면 가능 여부를 확인합니다. 다만 야간에 작업을 마친다고 다음 날 아침 모든 활동이 가능하다고 단정할 수는 없습니다. 실제 건조와 사용 조건을 고려해 작업 시간을 정해야 합니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">영업 중인 공간도 가능한가요?</h3>
            <div className="mt-2 rounded-xl bg-gray-50 p-5">
              <p className="text-[15.5px]">구역을 나누어 작업할 수 있는지와 출입 통제가 가능한지 먼저 확인합니다. 작업 구역을 사람들이 계속 통과해야 한다면 일정이나 작업 방식을 조정해야 할 수 있습니다.</p>
              <p className="mt-2 text-[15.5px]">현장 조건에 따라 전체 휴무가 필요한지, 부분 작업이 가능한지 상담합니다.</p>
            </div>
          </section>
          ...
          <section id="checkup" className="scroll-mt-36">
            ...
            <p>검수는 합의한 구역과 마감 계획을 기준으로 진행합니다. 광택뿐 아니라 작업 누락과 마감 상태, 사용 안내까지 함께 확인해 주세요.</p>
            ...
            <p className="mt-2">사용한 바닥재와 코팅 제품의 관리 기준에 맞춰 청소해 주세요. 물걸레질을 시작할 시점과 사용할 세정제, 피해야 할 관리 방법을 확인하는 것이 좋습니다.</p>
            <p className="mt-2 text-[15px] text-gray-500">집기는 안내받은 시점 이후에 배치하고, 이동할 때 바닥을 끌어 손상시키지 않도록 주의해 주세요. 의자 다리와 바퀴 등 반복적으로 바닥에 닿는 부분의 상태도 함께 살펴보시면 좋습니다.</p>

            <h3 className="mt-6 text-lg font-bold text-brand-dark">재코팅 주기는 공간마다 다릅니다</h3>
            <p className="mt-2">방문객 수, 이동 동선, 의자 사용, 청소 방법에 따라 코팅층이 닳는 속도가 달라집니다. 모든 공간에 같은 재코팅 주기를 적용하지는 않습니다.</p>
            <p className="mt-4 text-[15px] text-gray-500">통행이 많은 구역의 변화와 청소 후 상태를 보며 필요한 관리 시점을 상담해 주세요. 사후 문의와 재확인 범위·기간은 계약 시 확인하시기 바랍니다.</p>
          </section>
          ...
          <section id="contact" className="scroll-mt-36">
            <div className="rounded-2xl bg-brand-dark p-7 text-white">
              <p className="text-xl font-bold">바닥왁스코팅 견적 문의</p>
              <p className="mt-3 text-white/80">지금 바닥에 코팅이 필요한지, 박리부터 해야 하는지 모르셔도 괜찮습니다. 전체 사진과 상태가 잘 보이는 사진을 보내주세요.</p>
              <p className="mt-2 text-white/80">찐청소는 바닥 상태에 맞는 준비 작업과 코팅 범위를 먼저 확인합니다. 반짝이는 마감뿐 아니라, 다시 사용하는 일정과 관리 방법까지 함께 상담하세요.</p>
