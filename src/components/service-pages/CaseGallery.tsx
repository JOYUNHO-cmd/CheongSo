    ...
    >
      <div className="grid min-h-0 flex-1 grid-cols-2">
        <div className="relative flex min-h-0 items-center justify-center overflow-hidden bg-gray-100">
          <Image src={`/images/portfolio-v2/${item.before}`} alt={`${item.title} 시공 전`} width={item.beforeWidth} height={item.beforeHeight} className="h-full w-full object-contain" sizes="(min-width: 640px) 25vw, 45vw" />
          <Badge label="전" variant="before" />
        </div>
        <div className="relative flex min-h-0 items-center justify-center overflow-hidden bg-gray-100">
          <Image src={`/images/portfolio-v2/${item.after}`} alt={`${item.title} 시공 후`} width={item.afterWidth} height={item.afterHeight} className="h-full w-full object-contain" sizes="(min-width: 640px) 25vw, 45vw" />
