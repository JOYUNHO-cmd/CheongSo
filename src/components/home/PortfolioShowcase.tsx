...
export default function PortfolioShowcase() {
  ...
  return (
    <>
      <div className="scrollbar-hide flex gap-4 overflow-x-auto px-6 pb-4">
        {items.map((item) => (
          <button
            key={item.id}
            type="button"
            onClick={() => setOpenItem(item)}
            className="group w-64 shrink-0 rounded-2xl border border-gray-100 bg-white text-left shadow-sm transition-all hover:-translate-y-1 hover:border-brand hover:shadow-md"
          >
            <div className="grid grid-cols-2">
              <div className="relative aspect-square overflow-hidden rounded-tl-2xl bg-gray-100">
                <Image src={`/images/portfolio-gallery/${item.before}`} alt={`${item.title} 시공 전`} fill className="object-cover" sizes="128px" />
              ...
              <div className="relative aspect-square overflow-hidden rounded-tr-2xl bg-gray-100">
                <Image src={`/images/portfolio-gallery/${item.after}`} alt={`${item.title} 시공 후`} fill className="object-cover" sizes="128px" />
          ...
          <div className="flex max-h-full max-w-4xl flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
            <div className="grid w-full grid-cols-2 gap-2 md:gap-4">
              <div className="relative aspect-square overflow-hidden rounded-xl">
                <Image src={`/images/portfolio-gallery/${openItem.before}`} alt={`${openItem.title} 시공 전`} fill className="object-cover" sizes="50vw" />
                <span className="absolute left-2 top-2 rounded bg-gray-900/75 px-2 py-1 text-xs font-bold text-white">전</span>
              </div>
              <div className="relative aspect-square overflow-hidden rounded-xl">
                <Image src={`/images/portfolio-gallery/${openItem.after}`} alt={`${openItem.title} 시공 후`} fill className="object-cover" sizes="50vw" />
