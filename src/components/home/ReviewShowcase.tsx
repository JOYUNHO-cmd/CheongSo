...
import reviews from "@/lib/review-manifest.json";
...
export default function ReviewShowcase() {
  ...
  function next() {
    if (moving) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) setStart(value => (value + 3) % reviews.length);
    else setMoving(true);
  }
  ...
  return (
    <section aria-label="고객 후기 모아보기" aria-roledescription="캐러셀" className="mx-auto max-w-6xl px-6">
          ...
          {[0, 1].map(panel => (
            <div key={panel} className="review-curtain-panel" aria-hidden={panel === 1} inert={panel === 1}>
              {Array.from({ length: Math.min(3, reviews.length) }, (_, index) => {
                const position = (start + panel * 3 + index) % reviews.length;
                ...
                return <button key={index} type="button" onClick={() => setOpenFile(review.file)} className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white text-left shadow-sm transition-shadow hover:shadow-lg focus-visible:outline-2 focus-visible:outline-brand" aria-label={`고객 후기 ${position + 1} 크게 보기`}>
                  <div className="relative min-h-0 flex-1 overflow-hidden bg-gray-50"><Image src={`/images/reviews-v2/${review.file}`} alt={`실제 고객 후기 ${position + 1}`} width={review.width} height={review.height} className="h-full w-full object-contain" sizes="(min-width: 768px) 33vw, 90vw" /></div>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-sm">
        <p className="text-gray-500">후기 3개씩 · 선택하면 크게 읽을 수 있어요.</p>
