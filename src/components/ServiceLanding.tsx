  ...
  return <article>
    ...
    <div className="mx-auto grid max-w-5xl gap-8 px-6 py-12 md:grid-cols-[170px_1fr]">
      ...
      <div className="space-y-12 text-base leading-8 text-gray-700">
        ...
        <section id="faq" className="scroll-mt-36"><h2 className="text-2xl font-black text-brand-dark">{service.name} 자주 묻는 질문</h2>{[["상담 전에 무엇을 준비하면 좋나요?", `${service.check}${hasBatchim(service.check) ? "을" : "를"} 알려주세요. 작업 대상의 전체 사진과 오염 부분 사진이 있으면 범위를 확인하는 데 도움이 됩니다`], ["신청하면 모든 작업이 포함되나요?", service.limitation], ["작업 시간과 비용은 어떻게 정하나요?", "면적만으로 확정하지 않고 오염과 소재, 필요한 인원·장비, 출입 가능한 시간을 함께 확인합니다. 작업 전 포함 범위와 완료 확인 방법을 협의해 주세요"]].map(([q, a]) => <details key={q} className="mt-4 rounded-xl border border-gray-200 p-4"><summary className="cursor-pointer font-bold">{q}</summary><p className="mt-3">{a}</p></details>)}</section>
