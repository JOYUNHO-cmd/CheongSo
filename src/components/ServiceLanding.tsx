import readability from "./service-pages/Readability.module.css";
import { ReadingParagraph } from "./service-pages/ReadingParagraph";
import { BackToContents } from "./service-pages/BackToContents";
...
export default function ServiceLanding({ service, regional }: { service: ServiceProfile; regional?: RegionalPage }) {
  ...
  const introText = regional?.intro || service.intro;
  const introParts = introText.split(". ").filter(Boolean);

  return <article className={`${readability.landing} ${readability.enhanced}`}>
    ...
    <section className="bg-gradient-to-br from-brand-dark to-brand px-6 py-14 text-white md:py-20">
      <div className="mx-auto max-w-5xl">
        ...
        <ReadingParagraph className="text-sm font-bold tracking-widest text-brand-light">{category.title}</ReadingParagraph>
        <h1 className="mt-4 text-3xl font-black md:text-5xl">{heading}</h1>
        <ReadingParagraph className="mt-5 text-xl font-bold md:text-2xl">{service.headline}</ReadingParagraph>
        <div className="mt-5 max-w-3xl text-base sm:text-lg leading-relaxed text-white/90">
          {introParts.length === 2 ? (
            <>
              <span className="block">{introParts[0].replace(/\.+$/, "")}</span>
              <span className="block mt-1 sm:mt-1.5">{introParts[1].replace(/\.+$/, "")}</span>
            </>
          ) : (
            <ReadingParagraph className="leading-relaxed">{introText.replace(/\.+$/, "")}</ReadingParagraph>
          )}
        </div>
    ...
    <div className={readability.layout}>
      ...
      <div className={`${readability.body} space-y-12 text-gray-700`}>
        ...
        <section id="estimate" className="scroll-mt-36">
          ...
          <ReadingParagraph className="mt-3 text-[13.5px] sm:text-[14px] md:text-[14.5px] leading-relaxed tracking-tight text-gray-700">
            <span className="block">기본 작업, 추가 작업, 포함되지 않는 항목을 나눠 확인해주세요</span>
            <span className="block mt-1">현장 상태가 예상과 다르면 진행 전에 범위와 비용을 다시 협의합니다</span>
          </ReadingParagraph>
          <Link href="/pricing/" className="mt-4 inline-block font-bold text-brand">견적 기준 자세히 보기 →</Link>
        <BackToContents />
          </section>
        ...
        <section id="related" className="scroll-mt-36"><h2 className="text-2xl font-black text-brand-dark">필요한 관리, 함께 살펴보세요</h2><div className="mt-5 flex flex-wrap gap-3">{serviceProfiles.filter(s => s.category === service.category && s.slug !== service.slug).map(s => <Link className="rounded-full bg-brand-light px-4 py-2 text-sm font-bold text-brand-dark" key={s.slug} href={servicePath(s.name)}>{s.name} →</Link>)}</div>{!regional && regionalPages.filter(p => p.service === service.slug).map(p => <ReadingParagraph className="mt-5" key={p.region}><Link href={regionalPath(p)} className="font-bold text-brand">{p.heading} 지역 안내 →</Link></ReadingParagraph>)}<BackToContents />
          </section>
        <div className="rounded-2xl bg-brand-dark p-7 text-white"><ReadingParagraph className="text-xl font-bold">청소가 필요한 곳, 구체적으로 알려주세요</ReadingParagraph><ReadingParagraph className="mt-3 text-white/80">어떤 공간인지, 어디가 고민인지부터 정리하면 상담이 한결 쉬워집니다</ReadingParagraph><Link href="/contact/" className="mt-5 inline-block rounded-full bg-white px-5 py-2 font-bold text-brand-dark">상담 내용 정리하기 →</Link></div>
        <BackToTopButton />
      </div>
    </div>
  </article>;
