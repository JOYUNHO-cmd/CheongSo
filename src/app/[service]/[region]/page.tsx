...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const p = await getPage(params); return { title: p.title, description: p.description, alternates: { canonical: absoluteUrl(regionalPath(p)) }, openGraph: { title: p.title, description: p.description, url: absoluteUrl(regionalPath(p)), locale: "ko_KR", type: "website" } }; }
